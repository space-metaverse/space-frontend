    ...
    <Box sx={{ backgroundColor: '#111114', p: 5 }}>
      <Grid container>
        <Grid xs={12} sm={6} md={3}>
          <Box sx={{ p: 3 }}>
            <Image src={logoImage} alt='logo' style={{ width: '8rem', height: 'auto', objectFit: 'cover', background: 'white', borderRadius: 60 }} />
          </Box>
        </Grid>
        {
          footerItems.map((item, index) => (
            <Grid xs={12} sm={6} md={2} key={item.title + index}>
              <Box sx={{ p: 3 }}>
                <Typography variant="h5" fontWeight={500} sx={{ color: '#71717A' }}>
                  {item.title}
                </Typography>
                <Box sx={{ mt: 2 }}>
                  {
